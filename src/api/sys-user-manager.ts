...
export function queryUserList(params: UserParams) {
    return axios.post<ApiResponse<PageDataResponse<UserRecord>>>(
        '/api/user/list',
...
export function createUser(params: UserRecord) {
    return axios.post<ApiResponse<string>>('/api/user/create', params)
}

// 删除用户
export function deleteUser(id: number) {
    return axios.get<ApiResponse<string>>(`/api/user/delete?id=${id}`)
}
...
export function switchUserStatus(id: number) {
    return axios.get<ApiResponse<string>>(`/api/user/switchStatus?id=${id}`)
}

// 获取用户的角色列表
export function getUserRole(userId: number) {
    return axios.get<ApiResponse<string>>(`/api/user/roles?id=${userId}`)
}
